...
# Data/Benchmarks/KinoPaxStarComboTuning/<env>/ so they can be plotted independently.
#
...
# to change: the build/run matrix, the per-delta binary caching and --single-point all still loop.
DELTA_LABELS=("large")
DELTA_W_R1S=(10)
...
# DELTA_EXTRA_ARGS=("" "--single-point" "--single-point")
...
echo "  COMBO:          kAcc {1,4,16} x kFan {0,1,4,16} x rf 0.1 = 12 points"
echo "                  TWO shapes now: acceptance (which nodes join) and fan-out (where"
echo "                  propagation goes). kFan is the headline axis -- kFan 0 is the UNIFORM"
echo "                  fan-out control arm (CleanCost/KinoPaxPlus behaviour); high kFan makes"
echo "                  the shape bimodal, which concentrates fan-out KPAX-style at an"
echo "                  unchanged propagation budget."
...
if [ "$SKIP_BUILD" = false ]; then
    for d in "${!DELTA_LABELS[@]}"; do
        ...
        for i in "${!COST_LABELS[@]}"; do
            ...
            make KinoPaxStarComboTuningSweep -j"$(nproc)" 2>&1 | tail -20
            # Cache under a (delta, metric)-suffixed name so the run phase needs no rebuild
            cp KinoPaxStarComboTuningSweep "KinoPaxStarComboTuningSweep_${DL}_${CL}"
            cd "$PROJECT_DIR"
        done
    done
else
    ...
    for DL in "${DELTA_LABELS[@]}"; do
        for CL in "${COST_LABELS[@]}"; do
            if [ ! -f "KinoPaxStarComboTuningSweep_${DL}_${CL}" ]; then
                echo "ERROR: Cached binary not found: KinoPaxStarComboTuningSweep_${DL}_${CL}"
...
for CL in "${COST_LABELS[@]}"; do
    for i in "${!ENV_NAMES[@]}"; do
        ...
        for d in "${!DELTA_LABELS[@]}"; do
            ...
            # shellcheck disable=SC2086
            "./KinoPaxStarComboTuningSweep_${DL}_${CL}" "${DL}_${CL}" "$EO" "$EN" $PASS_FLAGS
        done
    done
done
cd "$PROJECT_DIR"
...
for EN in "${ENV_NAMES[@]}"; do
    echo "Results in: $BUILD_DIR/Data/Benchmarks/KinoPaxStarComboTuning/${EN}/"
done
...
if [ "${DUMP_VIZ:-0}" != "0" ]; then
    echo "Viz dumps:  $BUILD_DIR/Data/Benchmarks/KinoPaxStarComboTuning/viz/  (visualize with scripts/visualize_tree_growth.m)"
fi
