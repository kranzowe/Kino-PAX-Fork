#!/bin/bash
# =============================================================================
# KinoPaxSTAR Cost Benchmark Runner
#
# Cost-focused sibling of run_large_benchmark.sh: builds ONE config (the Large
# delta) and runs the KinoPaxStarCostBenchmark once, which executes the six
# planner families (KPAX, PruneKPAX, KinoPaxPlus, KinoPaxSTAR, and the two
# cost-prune cap/exp sweeps -- KinoPaxSTARcostprune with R2 sub-region seeding on,
# and KinoPaxSTARcostprunenoseed with it off) 5x each on the House environment.
# The two sweeps use the same (cap, exp) grid, so the only difference between
# a KinoPaxSTARcostprune_capN run and its KinoPaxSTARcostprune_noseed_capN
# counterpart is the R2 seeding free pass.
#
...
#
# Usage:
#   cd scripts && bash run_cost_benchmark.sh
# =============================================================================
set -euo pipefail
...
echo ""
echo "======================================================="
echo "  KinoPaxSTAR Cost Benchmark"
...
echo "  Planners: KPAX, PruneKPAX, KinoPaxPlus, KinoPaxSTAR,"
echo "            KinoPaxSTARcostprune sweep (seeded) x5 caps/exps,"
echo "            KinoPaxSTARcostprunenoseed sweep (pSeed=0) x5 caps/exps"
echo "======================================================="

# =============================================================================
# BUILD — compile the Large delta config
# =============================================================================
echo ""
echo "=== BUILDING (delta=${DELTA_LABEL}, Regions=${REGIONS}) ==="

write_config "$DELTA_W_R1" "$DELTA_C_R1" "$DELTA_V_R1"

cd "$BUILD_DIR"
# shellcheck disable=SC2086
cmake .. -DCMAKE_BUILD_TYPE=Release $CMAKE_COMPILER_FLAGS 2>&1 | tail -5
make KinoPaxStarCostBenchmark -j"$(nproc)" 2>&1 | tail -20
cd "$PROJECT_DIR"

# =============================================================================
# RUN — execute the benchmark (6 planner families, single delta, single env)
# =============================================================================
echo ""
...
# R1-density visualization. On by default; disable with DUMP_VIZ=0 bash run_cost_benchmark.sh
VIZ_FLAG=""
if [ "${DUMP_VIZ:-1}" != "0" ]; then
    VIZ_FLAG="--dump-viz"
fi
./KinoPaxStarCostBenchmark "$DELTA_LABEL" "$ENV_OBSTACLES" "$ENV_NAME" $VIZ_FLAG
cd "$PROJECT_DIR"

echo ""
echo "======================================================="
echo "  COST BENCHMARK COMPLETE"
echo "======================================================="
echo "Results in: $BUILD_DIR/Data/Benchmarks/KinoPaxStarCost/"
if [ "${DUMP_VIZ:-1}" != "0" ]; then
    echo "Viz dumps:  $BUILD_DIR/Data/Benchmarks/KinoPaxStarCost/viz/  (visualize with scripts/visualize_tree_growth.m)"
fi
