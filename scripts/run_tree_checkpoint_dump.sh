#!/bin/bash
# =============================================================================
# Tree Checkpoint Dump Runner
#
# Builds and runs TreeCheckpointDump, which dumps each planner's full tree --
# AND its solution trajectory with cost, if one exists yet -- at fixed
# WALL-CLOCK checkpoints (50ms, 100ms, 150ms, 1000ms, 4000ms of cumulative
# planner-only time) for three planners on the zigzag corridor:
#
...
# script assume MODEL 1 in [0,1]^3. Original config.h is backed up and restored
# on exit.
#
# Discretization: V2's canonical Model-1 "large"/coarse point (W7/C1[inert]/V3
# -> 9,261 regions -- see scripts/run_paper_benchmark_v2.sh's own derivation),
# NOT run_tree_growth_dump.sh's own, different "Large" (W10/C1/V3 -> 27,000) --
# confirmed with the user this tool should match V2's canonical point instead.
#
# Cost metric: COST_MODE 2 (path TIME), matching the metric this whole PaperPlotting
# branch's V2 work has standardized on, so results are comparable. Change
# COST_MODE_VAL below for control effort (1) or workspace distance (0).
#
# Usage:
#   cd scripts && bash run_tree_checkpoint_dump.sh
# =============================================================================
set -euo pipefail
...
# Single delta: label W_R1 C_R1 V_R1  (V2's canonical "large"/coarse point)
DELTA_LABEL="large"
DELTA_W_R1=7
DELTA_C_R1=1
DELTA_V_R1=3
...
echo ""
echo "======================================================="
echo "  TREE CHECKPOINT DUMP COMPLETE"
echo "======================================================="
echo "Trees/trajectories in:  $BUILD_DIR/Data/Viz/TreeCheckpoints/"
echo "Plot with: scripts/plot_tree_checkpoint.m"
echo "           cd $BUILD_DIR/Data/Viz/TreeCheckpoints"
echo "           matlab -batch \"addpath('$PROJECT_DIR/scripts'); plot_tree_checkpoint\""
