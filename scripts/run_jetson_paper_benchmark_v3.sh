#!/bin/bash
# =============================================================================
# JETSON PAPER BENCHMARK V3 -- a duplicate of run_paper_benchmark_v3.sh /
# examples/gpu/jetson_paper_benchmark_v3.cu (that latter file is its own duplicate of
# paper_benchmark_v3.cu; see its header), NOT a modification of v3 (v3 is untouched), adapted to
# actually run on a Jetson. Four changes from v3:
#
#   1. DISCRETIZATION FIXED TO "fine" FOR ALL THREE MODELS -- v3 ran all three deltas
#      (large/fine/tiny) per model (9 rows); this file only defines the 3 `fine` rows (one per
#      model, below), so COARSE/ and TINY/ output subfolders are simply never created. Same
#      MODEL_DELTA_*/delta_folder_name() mechanism as v3, just fewer active rows.
#   2. RUN COUNT 30 -> 10 per planner (NUM_KPAX_RUNS etc. in jetson_paper_benchmark_v3.cu).
#   3. TEGRA-AWARE BUILD PARALLELISM CAP, pulled from run_jetson_smoke_test.sh (this branch's own
#      prior Jetson work): `make -j"$(nproc)"` unconditionally, which every other run_*.sh in this
#      repo uses, will very likely OOM a Jetson's host compiler when building KPAX_lib's 30+ CUDA
#      files with CUDA_SEPARABLE_COMPILATION ON. Capped at -j2 when /etc/nv_tegra_release exists
#      (present on every JetPack image, nowhere else) -- see BUILD_JOBS below.
#   4. EXPLICIT CUDA-ERROR DETECTION. jetson_paper_benchmark_v3.cu now calls cudaGetLastError()
#      once per iteration in every planner loop and exits with a distinct code (2) on any CUDA
#      error, instead of relying solely on the external `timeout` wrapper's crash/hang detection --
#      a sticky CUDA error (cudaErrorIllegalAddress in particular, a DOCUMENTED real failure mode
#      "under Quad on Jetson" in this repo's own history) does not always crash the host process,
#      so it could otherwise silently corrupt a run's data with no failure ever logged. This
#      script's exit-code classification (RUN phase, below) recognizes exit code 2 as CUDA_ERROR.
#
# MAX_TREE_SIZE (1,000,000) and MAX_TIME_MS (3s) are UNCHANGED from v3 -- already Jetson-sized
# when v3 was created, not something this file needed to touch again. CS_RAMP_FILL_ITERS (210) is
# likewise unchanged in the .cu file, since it was derived from those same two constants -- see its
# comment there for the caveat that its 700-iteration empirical baseline was measured on cluster
# GPU throughput, not Jetson's.
#
# Everything else -- the four planners, all three vehicle models (Double Integrator/Dubins
# Airplane/Quad), 4 environments, 2 cost metrics, per-planner process isolation wrapped in an
# external `timeout` -- is identical to v3/v2. See run_paper_benchmark_v2.sh's own header
# (unchanged, not duplicated here) for the full MODEL 1->2->3 derivation history, the Dubins
# dimension-breakdown correction, and the CountingStars v2->v3.5 algorithm design history (an
# unrelated "v2/v3" -- that's the CountingStars algorithm's own version numbering inside
# CountingStars.cuh, not this script's).
#
# ALL THREE VEHICLE MODELS AT THE SINGLE "fine" DISCRETIZATION THIS PASS (MODEL_DELTA_* below, 3
# rows), 10 RUNS PER PLANNER, and each planner isolated to its OWN PROCESS invocation wrapped in
# an external `timeout` (PLANNER_FLAGS/PLANNER_NAMES below) -- a true CUDA hang has no in-process
# recovery (see the KPAX buffer-overshoot history further below), so this is what actually lets a
# hang be detected, logged as a failure, and skipped past rather than freezing the whole sweep.
# Per (model, environment, cost metric):
#   KPAX                                                    = 1 point  x 10 runs
#   KinoPaxPlus                                             = 1 point  x 10 runs
...
#                   hopelessGuard PERMANENTLY ON (v3.5)      = 1 point  x 10 runs
#
# BOTH COST METRICS (length + effort), FOUR ENVIRONMENTS (empty, house, narrowPassage, zigzag --
# see ENV_NAMES below), one build per (model, cost metric): 3 rows x 4 series x 10 runs x 4
# environments x 2 cost metrics = 960 runs total, from 6 compiled binaries, run as 96 separate
...
# written to its own subfolder under Data/Benchmarks/PaperBenchmarkV3Jetson/FINE/<env>/ -- own root
# from v3's PaperBenchmarkV3 (see jetson_paper_benchmark_v3.cu's outputDir comment for why: run
# count and hardware both differ, so pooling risks silently overwriting cluster data), same
# delta-folder-then-environment nesting convention, just always FINE here.
#
...
# metric) row (delta is fixed to "fine" for every row here, unlike v3), caching each binary under a
# suffixed name, then run them in a second pass. Labels ride into every output filename via the
# argv[1] TAG (m1_fine_length / m2_fine_effort / m3_fine_length / ... -- see the model x delta axis
# further below), which is what keeps different models' per-run CSVs from colliding within an
# environment folder (see the BUILD phase's own comment for why that matters) -- on top of that,
# delta ALSO splits the output at the top level, same mechanism as v3, but only ever into FINE/
# (see delta_folder_name() above).
#
# It builds ONLY the JetsonPaperBenchmarkV3 target. That still compiles KPAX_lib, which is the
...
#
# ALL THREE MODELS AT THE SINGLE "fine" DISCRETIZATION RUN THIS PASS -- v3 (the harness this file
# is a Jetson-adapted duplicate of) runs all three deltas per model; this file fixes every model to
# "fine" only. See the model x delta axis (MODEL_DELTA_MODEL_IDS/MODEL_DELTA_LABELS/
# MODEL_DELTA_W_R1S/MODEL_DELTA_C_R1S/MODEL_DELTA_V_R1S) further above for the derivation of all 3
# points.
#
...
#
# "fine" names the CELL, not the count, and the count differs by model -- see the model x delta
# axis further above for all 3 points (262,144 / 262,144 / 216,000). Every NUM_R1_REGIONS
# allocation and every full-array fill scales with this, and graph_.updateVertices() runs a kernel
# over all of them with 64 sub-vertex reads each.
#
# Original config.h is backed up and restored on exit/error.
#
# There is no automated cross-check script for this file's triplet (jetson_paper_benchmark_v3.cu /
# run_jetson_paper_benchmark_v3.sh / a Jetson-aware plot script, not yet created) the way
...
#
# Usage:
#   cd scripts && bash run_jetson_paper_benchmark_v3.sh
#   cd scripts && bash run_jetson_paper_benchmark_v3.sh --skip-build   # run only (cached binaries)
# =============================================================================
set -euo pipefail
...
# Model x delta axis -- v3 restored a full 3-point large/fine/tiny sweep per model (9 rows); THIS
# FILE FIXES DISCRETIZATION TO "fine" FOR ALL THREE MODELS, so only 3 rows exist here. Same
# flat-array idiom as v3 (bash has no clean 2D array), just fewer rows -- MODEL_DELTA_MODEL_IDS
# maps each row to its raw model id (1/2/3), MODEL_NAMES is still 0-indexed (DoubleIntegrator/
# DubinsAirplane/Quad at 0/1/2), so every lookup below uses `MODEL_NAMES[$((MODEL-1))]`.
#
# Numbers are IDENTICAL to v3's own "fine" rows (same verified sources -- see
# run_paper_benchmark_v3.sh's own copy of this comment for the full derivation and cross-check):
#
#   Model 1 (Double Integrator, W^3*V^3):    fine W16/C1/V4 -> 262,144
#   Model 2 (Dubins Airplane,  W^3*C^2*V):   fine W16/C4/V4 -> 262,144
#   Model 3 (Quad,             W^3*C^3*V^3): fine W6/C2/V5  -> 216,000
#
# KPAX BUFFER-OVERSHOOT HISTORY (NOT LIVE HERE): KPAX has a confirmed buffer-overshoot bug
# (h_treeSize_ exceeding MAX_TREE_SIZE via propagateFrontier()'s h_propIterations_==0 edge case,
# KPAX.cu:254-271) that hit a cudaErrorIllegalAddress crash under Quad on Jetson specifically at
# Quad's old, larger tiny point (7,2,6 -> 592,704) -- NOT run here, since this file never runs
# tiny at all. Still worth knowing given this file IS the Jetson target: see
# jetson_paper_benchmark_v3.cu's new explicit cudaGetLastError() checks (one per iteration, every
# planner) for the general-purpose defense against a recurrence of this class of bug at any
# discretization, not just the one that originally triggered it.

MODEL_NAMES=("DoubleIntegrator" "DubinsAirplane" "Quad")   # indexed by raw model id (1/2/3)

MODEL_DELTA_MODEL_IDS=(1    2    3)
MODEL_DELTA_LABELS=(fine fine fine)
MODEL_DELTA_W_R1S=(16   16   6)
MODEL_DELTA_C_R1S=(1    4    2)   # inert for Model 1 (C_DIM 0)
MODEL_DELTA_V_R1S=(4    4    5)

# Output-folder name per delta label -- same mechanism as v3 (deltaFolder CLI arg into
# jetson_paper_benchmark_v3.cu). large/tiny cases are unreachable with the fixed-fine rows above,
# kept so this stays a drop-in match for v3's delta_folder_name() if a future pass widens the axis
# back out.
delta_folder_name() {
    case "$1" in
        large) echo "COARSE" ;;
...
# --- Cap parallel compilation on Tegra. Every other scripts/run_*.sh (including v3's own
# run_paper_benchmark_v3.sh) uses `make -j"$(nproc)"` unconditionally; on a 6-core Orin Nano with
# 8 GB shared RAM, compiling KPAX_lib's 30+ CUDA files with CUDA_SEPARABLE_COMPILATION ON at full
# parallelism will very likely OOM the host (not the GPU -- the compiler). Pulled from this
# branch's own run_jetson_smoke_test.sh. /etc/nv_tegra_release exists on every JetPack image and
# nowhere else. ---
...
echo ""
echo "======================================================="
echo "  JETSON PAPER BENCHMARK V3 -- fixed 4-planner comparison, on countingstars_sweep.cu's harness"
echo "  ALL THREE MODELS AT THE SINGLE fine DISCRETIZATION this pass: ${MODEL_NAMES[*]}"
echo "  MAX_TREE_SIZE=1,000,000, per-run wall-clock cap 3s (unchanged from v3); 10 runs/planner (v3: 30)"
echo "  Build parallelism: -j${BUILD_JOBS} (capped on Tegra -- see file header)"
echo "  (Quad's obstacles/start/goal are scaled x100 from their [0,1]-authored CSVs -- see file header)"
echo "  Environments: ${ENV_NAMES[*]}  (nested inside the FINE output subfolder)"
...
echo "  Cost metrics: ${COST_LABELS[*]}  (one build each, per model row)"
...
echo "  whole sweep (see RUN_TIMEOUT_S / FAILURE_LOG below). jetson_paper_benchmark_v3.cu ALSO now"
echo "  checks cudaGetLastError() every iteration and exits distinctly (code 2, CUDA_ERROR below) on"
echo "  a sticky CUDA error that wouldn't otherwise crash the process. Series per (model, env, cost),"
echo "  10 runs each -- every axis below is a SINGLE FIXED POINT, not a grid:"
echo "    KPAX             baseline -- has a documented history of cudaErrorIllegalAddress under"
echo "                     Quad on Jetson at a much finer discretization than fine (see file header);"
echo "                     the new CUDA-error check above and the per-planner timeout below both"
echo "                     catch and log a recurrence rather than corrupting data or hanging silently."
echo "    KinoPaxPlus"
...
TOTAL_RUNS=$(( TOTAL_INVOCATIONS * 10 ))
echo "  = ${#MODEL_DELTA_LABELS[@]} model rows x ${#PLANNER_NAMES[@]} planners x 10 runs x ${#ENV_NAMES[@]} environments x ${#COST_LABELS[@]} cost"
...
if [ "$SKIP_BUILD" = false ]; then
    for row in "${!MODEL_DELTA_LABELS[@]}"; do
        ...
        for i in "${!COST_LABELS[@]}"; do
            ...
            make JetsonPaperBenchmarkV3 -j"$BUILD_JOBS" 2>&1 | tail -20
            # Cache under a (model, delta, metric)-suffixed name so the run phase needs no rebuild
            cp JetsonPaperBenchmarkV3 "JetsonPaperBenchmarkV3_${TAG}_${CL}"
            cd "$PROJECT_DIR"
        done
    done
else
    ...
    for row in "${!MODEL_DELTA_LABELS[@]}"; do
        ...
        for CL in "${COST_LABELS[@]}"; do
            if [ ! -f "JetsonPaperBenchmarkV3_${TAG}_${CL}" ]; then
                echo "ERROR: Cached binary not found: JetsonPaperBenchmarkV3_${TAG}_${CL}"
...
# external timeout. NOTHING inside the .cu file's own host loop can recover from a true HANG (a
# kernel that never returns): the same host thread that would check plannerMs >= maxTimeMs is the
# one parked inside cudaEventSynchronize if a single kernel launch genuinely wedges (see
# jetson_paper_benchmark_v3.cu's skipKPAXThisDelta comment for the historical hang this guards
# against). OS-level termination from outside the process is the only mechanism that can recover
# from THAT. A CRASH or silent CORRUPTION (a kernel that returns but leaves a poisoned CUDA
# context, e.g. cudaErrorIllegalAddress) is different and IS now caught in-process --
# jetson_paper_benchmark_v3.cu checks cudaGetLastError() every iteration and exits(2) on any error,
# classified below as CUDA_ERROR, rather than corrupting a run's data silently or waiting for the
# external timeout to eventually kill a process that isn't actually hung.
# =============================================================================
...
# RUN_TIMEOUT_S: RECOMPUTED FOR 10 runs/planner (v3: 30), same MAX_TIME_MS=3s and same formula as
# v3's own derivation: 10 runs x up to 3s MAX_TIME_MS + 9 x 0.5s inter-run sleeps = 34.5s pure
# planner-loop ceiling (the expected case, not a pessimistic one) -- DOUBLED for margin, ~69s,
# rounded to 70s, so a legitimately slow run is never misclassified as a failure. NOTE: this
# formula assumes Jetson achieves roughly the same per-iteration throughput the cluster does within
# the 3s cap -- if a first real run shows otherwise (Jetson's GPU is meaningfully weaker), raise
# this rather than assume every TIMEOUT in the failure log is a genuine hang.
RUN_TIMEOUT_S=70
KILL_AFTER_S=30   # grace period after SIGTERM before timeout escalates to SIGKILL

OUTPUT_ROOT="$BUILD_DIR/Data/Benchmarks/PaperBenchmarkV3Jetson"
...
for row in "${!MODEL_DELTA_LABELS[@]}"; do
    ...
    for i in "${!COST_LABELS[@]}"; do
        CL="${COST_LABELS[$i]}"
        BIN="JetsonPaperBenchmarkV3_${TAG}_${CL}"
        for e in "${!ENV_NAMES[@]}"; do
            ...
            for p in "${!PLANNER_NAMES[@]}"; do
                ...
                # the discretization output folder (DF, same mechanism as v3), read by
                # jetson_paper_benchmark_v3.cu before the --only-* flags -- see its own
                # argv-parsing comment.
                # shellcheck disable=SC2086
                ...
                if [ "$ec" -ne 0 ]; then
                    ...
                    # "this hung" unambiguously. Exit code 2 is jetson_paper_benchmark_v3.cu's OWN
                    # explicit cudaGetLastError() check firing (NEW for this Jetson variant, see
                    # file header) -- a real CUDA error was detected and the process exited itself,
                    # cleanly, rather than being killed from outside; check this BEFORE the >=128
                    # branch since 2 is not a signal-death code. ec>=128 (and not 2) is a genuine
                    # crash: the process died from a signal on its own, unrelated to this wrapper.
                    if   [ "$ec" -eq 124 ]; then STATUS="TIMEOUT"                   # hit RUN_TIMEOUT_S
                    elif [ "$ec" -eq 2   ]; then STATUS="CUDA_ERROR"                # cudaGetLastError() caught it
...
echo ""
echo "======================================================="
echo "  JETSON PAPER BENCHMARK V3 COMPLETE"
echo "======================================================="
for EN in "${ENV_NAMES[@]}"; do
    echo "Results in: $BUILD_DIR/Data/Benchmarks/PaperBenchmarkV3Jetson/FINE/${EN}/"
done
echo "No Jetson-aware plot script exists yet -- read the per-run CSVs directly, or adapt a"
echo "v3-aware one (also not yet created) to this layout: PaperBenchmarkV3Jetson/FINE/<env>/,"
echo "not v3's PaperBenchmarkV3/<COARSE|FINE|TINY>/<env>/ or v2's flat PaperBenchmarkV2/<env>/."
if [ "${DUMP_VIZ:-0}" != "0" ]; then
    echo "Viz dumps:  $BUILD_DIR/Data/Benchmarks/PaperBenchmarkV3Jetson/FINE/<env>/viz/  (visualize with scripts/visualize_tree_growth.m)"
fi
