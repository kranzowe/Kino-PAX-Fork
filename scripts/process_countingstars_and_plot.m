...
% sprintf('%s_%s', delta, metric), e.g. 'fine_length'. TWO THIS PASS -- "tiny" dropped, and both
% run the full comparison (see deltaPlusOnly below).
deltas      = {'large', 'fine'};
...
for ei = 1:numel(environments)
    ...
    for mi = 1:numel(metrics)
        ...
        %
        % bufferSlope/bufferFloor are FIXED this pass (1.2, 0.05), so EVERY series here should rise
        % along the SAME ramp shape -- roughly linearly from bufferFloor's starting value toward
        % (bufferSlope + bufferFloor) * MAX_TREE_SIZE / fill_iters at the last iteration. Any series
        % that visibly diverges from that shared shape is a wiring bug, not an intended contrast --
        % there is no "flat vs ramped" comparison to look for this pass, unlike when bufferSlope
        % itself was the swept axis.
        figNum = figNum + 1;
        ...
        %                          the SAME range at every series, since the ramp itself is not
        %                          what's swept. B still moves WITHIN a run (the ramp is unchanged),
        %                          just not across series any more.
        %
