...
KinoPaxSTARnoseed::KinoPaxSTARnoseed()
{
    graph_ = Graph(W_SIZE);
    ...
    if(VERBOSE)
        {
            printf("/* Planner Type: KinoPaxSTARnoseed (Hybrid) */\n");
...
void KinoPaxSTARnoseed::resetPlanner(float* h_initial, float* h_goal)
{
...
void KinoPaxSTARnoseed::plan(float* h_initial, float* h_goal, float* d_obstacles_ptr, uint h_obstaclesCount)
{
    ...
    std::cout << "KinoPaxSTARnoseed execution time: " << milliseconds / 1000.0 << " seconds. Iterations: " << h_itr_
              << ". Tree Size: " << h_treeSize_ << ". Best Cost: " << h_minCost_ << std::endl;
    cudaEventDestroy(start);
    cudaEventDestroy(stop);
}

void KinoPaxSTARnoseed::planBench(float* h_initial, float* h_goal, float* d_obstacles_ptr, uint h_obstaclesCount, int benchItr)
{
    ...
    std::cout << "KinoPaxSTARnoseed execution time: " << executionTime << " seconds. Iterations: " << h_itr_
              << ". Tree Size: " << h_treeSize_ << ". Best Cost: " << h_minCost_ << std::endl;
}

float KinoPaxSTARnoseed::planOptimize(float* h_initial, float* h_goal, float* d_obstacles_ptr, uint h_obstaclesCount)
{
    cudaEvent_t start, stop;
    float milliseconds = 0;
    cudaEventCreate(&start);
...
void KinoPaxSTARnoseed::propagateFrontier(float* d_obstacles_ptr, uint h_obstaclesCount)
{
    ...
    if(h_frontierRepeatSize_ * h_activeBlockSize_ > (MAX_TREE_SIZE - h_treeSize_))
        {
            ...
            KinoPaxSTARnoseed_propagateFrontier_kernel2<<<iDivUp(h_propIterations_ * h_frontierRepeatSize_, h_activeBlockSize_), h_activeBlockSize_>>>(
        ...
        {
            KinoPaxSTARnoseed_propagateFrontier_kernel1<<<iDivUp(h_frontierRepeatSize_ * h_activeBlockSize_, h_activeBlockSize_), h_activeBlockSize_>>>(
              d_frontier_ptr_, d_activeFrontierRepeatIdxs_ptr_, d_treeSamples_ptr_, d_unexploredSamples_ptr_, h_frontierRepeatSize_,
              d_randomSeeds_ptr_, d_unexploredSamplesParentIdxs_ptr_, d_obstacles_ptr, h_obstaclesCount, graph_.d_activeSubVertices_ptr_,
              graph_.d_vertexScoreArray_ptr_, d_frontierNext_ptr_, graph_.d_counterArray_ptr_, graph_.d_validCounterArray_ptr_,
...
__global__ void KinoPaxSTARnoseed_propagateFrontier_kernel1(bool* frontier, uint* activeFrontierIdxs, float* treeSamples,
    ...
    if(valid)
        {
            ...
            // Track 2: Exploration (KPAX vertex score only; new-sub-region seeding removed)
...
__global__ void KinoPaxSTARnoseed_propagateFrontier_kernel2(bool* frontier, uint* activeFrontierIdxs, float* treeSamples,
                                                   float* unexploredSamples, uint frontierSize, curandState* randomSeeds,
                                                   int* unexploredSamplesParentIdxs, float* obstacles, int obstaclesCount,
                                                   int* activeSubVertices, float* vertexScores, bool* frontierNext,
...
__global__ void
KinoPaxSTARnoseed_updateFrontier_kernel(bool* frontier, bool* frontierNext, uint* activeFrontierNextIdxs, uint frontierNextSize,
...
void KinoPaxSTARnoseed::updateFrontier()
{
    ...
    // --- Update Frontier ---
    thrust::fill(d_activeFrontierRepeatCount_.begin(), d_activeFrontierRepeatCount_.end(), 0);
    KinoPaxSTARnoseed_updateFrontier_kernel<<<iDivUp(h_frontierNextSize_ + h_treeSize_, h_blockSize_), h_blockSize_>>>(
...
/***************************/
void KinoPaxSTARnoseed::getControlPathToGoal()
{
    ...
    KinoPaxSTARnoseed_getControlPathToGoal_kernel<<<iDivUp(h_solSetSize_, h_blockSize_), h_blockSize_>>>(
...
__global__ void KinoPaxSTARnoseed_getControlPathToGoal_kernel(float* controlPathsToGoal, float* treeSamples,
...
void KinoPaxSTARnoseed::writeExecutionTimeToCSV(double time)
{
