...
KinoPaxSTARsparsefill::KinoPaxSTARsparsefill()
{
    graph_ = Graph(W_SIZE);
    ...
    // sparse->fill schedule: pSeed ramps 0->1 over the first h_rampIters_ iterations.
    // Early (pSeed~0): new-sub-region seeding suppressed -> sparse R1 skeleton.
    // Late  (pSeed->1): seeding on -> fills the empty R2 sub-cells.
    h_rampIters_ = 100.0f;

    if(VERBOSE)
        {
            printf("/* Planner Type: KinoPaxSTARsparsefill (Hybrid) */\n");
...
void KinoPaxSTARsparsefill::resetPlanner(float* h_initial, float* h_goal)
{
...
void KinoPaxSTARsparsefill::plan(float* h_initial, float* h_goal, float* d_obstacles_ptr, uint h_obstaclesCount)
{
    ...
    std::cout << "KinoPaxSTARsparsefill execution time: " << milliseconds / 1000.0 << " seconds. Iterations: " << h_itr_
              << ". Tree Size: " << h_treeSize_ << ". Best Cost: " << h_minCost_ << std::endl;
    cudaEventDestroy(start);
    cudaEventDestroy(stop);
}

void KinoPaxSTARsparsefill::planBench(float* h_initial, float* h_goal, float* d_obstacles_ptr, uint h_obstaclesCount, int benchItr)
{
    ...
    std::cout << "KinoPaxSTARsparsefill execution time: " << executionTime << " seconds. Iterations: " << h_itr_
              << ". Tree Size: " << h_treeSize_ << ". Best Cost: " << h_minCost_ << std::endl;
}

float KinoPaxSTARsparsefill::planOptimize(float* h_initial, float* h_goal, float* d_obstacles_ptr, uint h_obstaclesCount)
{
    cudaEvent_t start, stop;
    float milliseconds = 0;
    cudaEventCreate(&start);
...
void KinoPaxSTARsparsefill::propagateFrontier(float* d_obstacles_ptr, uint h_obstaclesCount)
{
    ...
    // --- Sparse->fill new-sub-region seed probability: ~0 early, ramps to 1 by h_rampIters_ ---
    float h_pSeed = std::min(1.0f, float(h_itr_) / h_rampIters_);
    ...
    if(h_frontierRepeatSize_ * h_activeBlockSize_ > (MAX_TREE_SIZE - h_treeSize_))
        {
            ...
            KinoPaxSTARsparsefill_propagateFrontier_kernel2<<<iDivUp(h_propIterations_ * h_frontierRepeatSize_, h_activeBlockSize_), h_activeBlockSize_>>>(
              ...
              d_treeSampleCosts_ptr_, d_minCostsR1_ptr_, d_frontierNextXR1s_ptr_, d_unexploredSampleCosts_ptr_, h_pSeed, h_spatialHashGrid_);
        }
    else
        {
            KinoPaxSTARsparsefill_propagateFrontier_kernel1<<<iDivUp(h_frontierRepeatSize_ * h_activeBlockSize_, h_activeBlockSize_), h_activeBlockSize_>>>(
              d_frontier_ptr_, d_activeFrontierRepeatIdxs_ptr_, d_treeSamples_ptr_, d_unexploredSamples_ptr_, h_frontierRepeatSize_,
              d_randomSeeds_ptr_, d_unexploredSamplesParentIdxs_ptr_, d_obstacles_ptr, h_obstaclesCount, graph_.d_activeSubVertices_ptr_,
              graph_.d_vertexScoreArray_ptr_, d_frontierNext_ptr_, graph_.d_counterArray_ptr_, graph_.d_validCounterArray_ptr_,
...
__global__ void KinoPaxSTARsparsefill_propagateFrontier_kernel1(bool* frontier, uint* activeFrontierIdxs, float* treeSamples,
                                                   ...
                                                   float* unexploredSampleCosts, float pSeed, SpatialHashGrid spatialHashGrid)
{
    ...
    if(valid)
        {
            ...
            // Track 2: Exploration (KPAX vertex score OR probabilistically-gated new sub-region)
...
__global__ void KinoPaxSTARsparsefill_propagateFrontier_kernel2(bool* frontier, uint* activeFrontierIdxs, float* treeSamples,
                                                   float* unexploredSamples, uint frontierSize, curandState* randomSeeds,
                                                   int* unexploredSamplesParentIdxs, float* obstacles, int obstaclesCount,
                                                   int* activeSubVertices, float* vertexScores, bool* frontierNext,
...
__global__ void
KinoPaxSTARsparsefill_updateFrontier_kernel(bool* frontier, bool* frontierNext, uint* activeFrontierNextIdxs, uint frontierNextSize,
...
void KinoPaxSTARsparsefill::updateFrontier()
{
    ...
    // --- Update Frontier ---
    thrust::fill(d_activeFrontierRepeatCount_.begin(), d_activeFrontierRepeatCount_.end(), 0);
    KinoPaxSTARsparsefill_updateFrontier_kernel<<<iDivUp(h_frontierNextSize_ + h_treeSize_, h_blockSize_), h_blockSize_>>>(
...
/***************************/
void KinoPaxSTARsparsefill::getControlPathToGoal()
{
    ...
    KinoPaxSTARsparsefill_getControlPathToGoal_kernel<<<iDivUp(h_solSetSize_, h_blockSize_), h_blockSize_>>>(
...
__global__ void KinoPaxSTARsparsefill_getControlPathToGoal_kernel(float* controlPathsToGoal, float* treeSamples,
...
void KinoPaxSTARsparsefill::writeExecutionTimeToCSV(double time)
{
