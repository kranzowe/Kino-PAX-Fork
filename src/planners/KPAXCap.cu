// KPAXCap -- stock KPAX with one addition: h_syclopCap_, a multiplier in (0,1] on the Syclop
// region score, applied at both acceptance points (propagate and dormant-node reactivation)
// with fAccept and the R2 seeding free pass left unscaled. cap = 1.0 is bit-identical to KPAX.
//
// It is the control arm for the cap: KinoPaxSTARCleanCost applies the same multiplier but also
// moved its acceptance decision past the kernel boundary, so comparing KPAX / KPAXCap /
// CleanCost-at-w=1 separates the cap's effect from the boundary's. See KPAXCap.cuh for detail.
...
KPAXCap::KPAXCap()
{
    ...
    h_syclopCap_       = 1.0f;   // 1.0 == stock KPAX exactly

    if(VERBOSE)
        {
            printf("/* Planner Type: KPAXCap */\n");
...
void KPAXCap::resetPlanner(float* h_initial, float* h_goal)
{
...
void KPAXCap::plan(float* h_initial, float* h_goal, float* d_obstacles_ptr, uint h_obstaclesCount)
{
    ...
    std::cout << "KPAXCap execution time: " << milliseconds / 1000.0 << " seconds. Iterations: " << h_itr_ << ". Tree Size: " << h_treeSize_
              << std::endl;
    cudaEventDestroy(start);
    cudaEventDestroy(stop);
}

void KPAXCap::planDebug(float* h_initial, float* h_goal, float* d_obstacles_ptr, uint h_obstaclesCount)
{
    cudaEvent_t start, stop;
    float milliseconds = 0;
    cudaEventCreate(&start);
    ...
    std::cout << "\n=== KPAXCap TIMING BREAKDOWN ===" << std::endl;
...
void KPAXCap::planBench(float* h_initial, float* h_goal, float* d_obstacles_ptr, uint h_obstaclesCount, int benchItr)
{
    double t_kgmtStart = std::clock();

    // --- KPAXCap INITIALIZATION ---
    resetPlanner(h_initial, h_goal);
    ...
    std::cout << "KPAXCap execution time: " << executionTime << " seconds. Iterations: " << h_itr_ << ". Tree Size: " << h_treeSize_
              << std::endl;
}

void KPAXCap::propagateFrontier(float* d_obstacles_ptr, uint h_obstaclesCount)
{
    ...
    if(h_frontierRepeatSize_ * h_activeBlockSize_ > (MAX_TREE_SIZE - h_treeSize_))
        {
            ...
            KPAXCap_propagateFrontier_kernel2<<<iDivUp(h_propIterations_ * h_frontierRepeatSize_, h_activeBlockSize_), h_activeBlockSize_>>>(
              ...
              h_propIterations_, graph_.d_minValueInRegion_ptr_, h_syclopCap_);
        }
    else
        {
            // --- Propagate Frontier. Block Size new samples per frontier sample. ---
            KPAXCap_propagateFrontier_kernel1<<<iDivUp(h_frontierRepeatSize_ * h_activeBlockSize_, h_activeBlockSize_), h_activeBlockSize_>>>(
              d_frontier_ptr_, d_activeFrontierRepeatIdxs_ptr_, d_treeSamples_ptr_, d_unexploredSamples_ptr_, h_frontierRepeatSize_,
              d_randomSeeds_ptr_, d_unexploredSamplesParentIdxs_ptr_, d_obstacles_ptr, h_obstaclesCount, graph_.d_activeSubVertices_ptr_,
              graph_.d_vertexScoreArray_ptr_, d_frontierNext_ptr_, graph_.d_counterArray_ptr_, graph_.d_validCounterArray_ptr_,
              graph_.d_minValueInRegion_ptr_, h_syclopCap_);
        }
}
    ...
    if(valid)
        {
            ...
            // Cap scales the Syclop score only; the seeding disjunct stays a free pass.
            if(curand_uniform(&randSeed) < syclopCap * vertexScores[x1Vertex] || !activeSubVertices[x1SubVertex]) frontierNext[tid] = true;
    ...
    if(valid)
        {
            atomicAdd(&validVertexCounter[x1Vertex], 1);
            // Cap scales the Syclop score only; the seeding disjunct stays a free pass.
            if(curand_uniform(&randSeed) < syclopCap * vertexScores[x1Vertex] || activeSubVertices[x1SubVertex] == 0) frontierNext[tid] = true;
            if(activeSubVertices[x1SubVertex] == 0) atomicExch(&activeSubVertices[x1SubVertex], 1);
        }
                      ...
                      curandState* randomSeeds, float* vertexScores, float* controlPathToGoal, float fAccept, float syclopCap)
{
    ...
    // --- Add inactive tree samples back to frontier. ---
    else if(tid < frontierNextSize + treeSize)
        {
            ...
            // fAccept stays UNSCALED so a small cap throttles reactivation rather than
            // switching it off -- it is the additive floor dormant nodes rely on.
            if(frontier[treeIdx] == 0 && curand_uniform(&seed) <= syclopCap * vertexScores[xVertex] + fAccept)
                {
...
void KPAXCap::updateFrontier()
{
    ...
    // --- Update Frontier ---
    thrust::fill(d_activeFrontierRepeatCount_.begin(), d_activeFrontierRepeatCount_.end(), 0);
    KPAXCap_updateFrontier_kernel<<<iDivUp(h_frontierNextSize_ + h_treeSize_, h_blockSize_), h_blockSize_>>>(
      ...
      graph_.d_vertexScoreArray_ptr_, d_controlPathToGoal_ptr_, h_fAccept_, h_syclopCap_);
...
void KPAXCap::writeDeviceVectorsToCSV(int itr)
{
...
void KPAXCap::writeExecutionTimeToCSV(double time)
{
    std::ostringstream filename;
    std::filesystem::create_directories("Data");
