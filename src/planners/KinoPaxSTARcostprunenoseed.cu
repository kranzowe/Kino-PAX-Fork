// KinoPaxSTARcostprunenoseed — KinoPaxSTARcostprune with the R2 sub-region seeding free pass removed.
// The cost-prune machinery (per-region cost stats, costKeepProb gate, _costPrune_kernel,
// cost-blended reactivation) is identical to KinoPaxSTARcostprune. The only difference is that
// a sample landing in a never-before-occupied R2 sub-region is no longer admitted unconditionally;
// it is admitted with probability h_pSeed_, which defaults to 0. Set h_pSeed_ = 1 to recover
// stock KinoPaxSTARcostprune behaviour.
#include "planners/KinoPaxSTARcostprunenoseed.cuh"
#include "config/config.h"
#include "statePropagator/statePropagatorSpatialHash.cuh"

KinoPaxSTARcostprunenoseed::KinoPaxSTARcostprunenoseed()
{
    graph_ = Graph(W_SIZE);
    ...
    //   pSeed          probability of admitting a sample purely because it landed in a
    //                  never-before-occupied R2 sub-region. 0 = that free pass is gone entirely
    //                  (the "noseed" behaviour of this variant); 1 = stock KinoPaxSTARcostprune.
    h_acceptCap_      = 0.1f;
    ...
    if(VERBOSE)
        {
            printf("/* Planner Type: KinoPaxSTARcostprunenoseed (Hybrid) */\n");
...
void KinoPaxSTARcostprunenoseed::resetPlanner(float* h_initial, float* h_goal)
{
...
void KinoPaxSTARcostprunenoseed::plan(float* h_initial, float* h_goal, float* d_obstacles_ptr, uint h_obstaclesCount)
{
    ...
    std::cout << "KinoPaxSTARcostprunenoseed execution time: " << milliseconds / 1000.0 << " seconds. Iterations: " << h_itr_
              << ". Tree Size: " << h_treeSize_ << ". Best Cost: " << h_minCost_ << std::endl;
    cudaEventDestroy(start);
    cudaEventDestroy(stop);
}

void KinoPaxSTARcostprunenoseed::planBench(float* h_initial, float* h_goal, float* d_obstacles_ptr, uint h_obstaclesCount, int benchItr)
{
    ...
    std::cout << "KinoPaxSTARcostprunenoseed execution time: " << executionTime << " seconds. Iterations: " << h_itr_
              << ". Tree Size: " << h_treeSize_ << ". Best Cost: " << h_minCost_ << std::endl;
}

float KinoPaxSTARcostprunenoseed::planOptimize(float* h_initial, float* h_goal, float* d_obstacles_ptr, uint h_obstaclesCount)
{
    cudaEvent_t start, stop;
    float milliseconds = 0;
    cudaEventCreate(&start);
...
void KinoPaxSTARcostprunenoseed::propagateFrontier(float* d_obstacles_ptr, uint h_obstaclesCount)
{
    ...
    if(h_frontierRepeatSize_ * h_activeBlockSize_ > (MAX_TREE_SIZE - h_treeSize_))
        {
            ...
            KinoPaxSTARcostprunenoseed_propagateFrontier_kernel2<<<iDivUp(h_propIterations_ * h_frontierRepeatSize_, h_activeBlockSize_), h_activeBlockSize_>>>(
              ...
              d_treeSampleCosts_ptr_, d_minCostsR1_ptr_, d_maxCostsR1_ptr_, d_sumCostsR1_ptr_, d_cntCostsR1_ptr_, d_frontierNextXR1s_ptr_, d_unexploredSampleCosts_ptr_, h_acceptCap_, h_pSeed_, h_spatialHashGrid_);
        }
    else
        {
            KinoPaxSTARcostprunenoseed_propagateFrontier_kernel1<<<iDivUp(h_frontierRepeatSize_ * h_activeBlockSize_, h_activeBlockSize_), h_activeBlockSize_>>>(
              d_frontier_ptr_, d_activeFrontierRepeatIdxs_ptr_, d_treeSamples_ptr_, d_unexploredSamples_ptr_, h_frontierRepeatSize_,
              d_randomSeeds_ptr_, d_unexploredSamplesParentIdxs_ptr_, d_obstacles_ptr, h_obstaclesCount, graph_.d_activeSubVertices_ptr_,
              graph_.d_vertexScoreArray_ptr_, d_frontierNext_ptr_, graph_.d_counterArray_ptr_, graph_.d_validCounterArray_ptr_,
...
__global__ void KinoPaxSTARcostprunenoseed_propagateFrontier_kernel1(bool* frontier, uint* activeFrontierIdxs, float* treeSamples,
                                                   ...
                                                   float* unexploredSampleCosts, float acceptCap, float pSeed, SpatialHashGrid spatialHashGrid)
{
    ...
    if(valid)
        {
            ...
            // Track 2: Exploration (KPAX vertex score; new-sub-region seeding gated on pSeed, 0 by default)
            bool acceptedByExploration = (curand_uniform(&randSeed) < fminf(vertexScores[x1Vertex], acceptCap))
                                       || (!activeSubVertices[x1SubVertex] && curand_uniform(&randSeed) < pSeed);
...
__global__ void KinoPaxSTARcostprunenoseed_propagateFrontier_kernel2(bool* frontier, uint* activeFrontierIdxs, float* treeSamples,
                                                   float* unexploredSamples, uint frontierSize, curandState* randomSeeds,
                                                   int* unexploredSamplesParentIdxs, float* obstacles, int obstaclesCount,
                                                   int* activeSubVertices, float* vertexScores, bool* frontierNext,
    ...
    if(valid)
        {
            ...
            // Track 2: Exploration (new-sub-region seeding gated on pSeed, 0 by default)
            bool acceptedByExploration = (curand_uniform(&randSeed) < fminf(vertexScores[x1Vertex], acceptCap))
                                       || (activeSubVertices[x1SubVertex] == 0 && curand_uniform(&randSeed) < pSeed);
...
__global__ void KinoPaxSTARcostprunenoseed_costPrune_kernel(uint* activeFrontierNextIdxs, uint frontierNextSize,
...
__global__ void
KinoPaxSTARcostprunenoseed_updateFrontier_kernel(bool* frontier, bool* frontierNext, uint* activeFrontierNextIdxs, uint frontierNextSize,
                               ...
                               int* iterations, int iteration)
{
    ...
    // --- Part B: Re-activate existing tree nodes ---
    else if(tid < frontierNextSize + treeSize)
        {
            ...
            // REACTIVATION: Non-best nodes re-enter via the cost gate (cost vs region min)
            // blended with the capped Syclop exploration score.
...
void KinoPaxSTARcostprunenoseed::updateFrontier()
{
    ...
    KinoPaxSTARcostprunenoseed_costPrune_kernel<<<iDivUp(h_frontierNextSize_, h_blockSize_), h_blockSize_>>>(
    ...
    // --- Update Frontier ---
    thrust::fill(d_activeFrontierRepeatCount_.begin(), d_activeFrontierRepeatCount_.end(), 0);
    KinoPaxSTARcostprunenoseed_updateFrontier_kernel<<<iDivUp(h_frontierNextSize_ + h_treeSize_, h_blockSize_), h_blockSize_>>>(
...
/***************************/
void KinoPaxSTARcostprunenoseed::getControlPathToGoal()
{
    ...
    KinoPaxSTARcostprunenoseed_getControlPathToGoal_kernel<<<iDivUp(h_solSetSize_, h_blockSize_), h_blockSize_>>>(
...
__global__ void KinoPaxSTARcostprunenoseed_getControlPathToGoal_kernel(float* controlPathsToGoal, float* treeSamples,
...
void KinoPaxSTARcostprunenoseed::writeExecutionTimeToCSV(double time)
{
