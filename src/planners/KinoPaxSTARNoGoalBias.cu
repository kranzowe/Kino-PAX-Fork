...
KinoPaxSTARNoGoalBias::KinoPaxSTARNoGoalBias()
{
    graph_ = Graph(W_SIZE);
    ...
    if(VERBOSE)
        {
            printf("/* Planner Type: KinoPaxSTARNoGoalBias (Hybrid) */\n");
...
void KinoPaxSTARNoGoalBias::resetPlanner(float* h_initial, float* h_goal)
{
...
void KinoPaxSTARNoGoalBias::plan(float* h_initial, float* h_goal, float* d_obstacles_ptr, uint h_obstaclesCount)
{
    ...
    std::cout << "KinoPaxSTARNoGoalBias execution time: " << milliseconds / 1000.0 << " seconds. Iterations: " << h_itr_
              << ". Tree Size: " << h_treeSize_ << ". Best Cost: " << h_minCost_ << std::endl;
    cudaEventDestroy(start);
    cudaEventDestroy(stop);
}

void KinoPaxSTARNoGoalBias::planBench(float* h_initial, float* h_goal, float* d_obstacles_ptr, uint h_obstaclesCount, int benchItr)
{
    ...
    std::cout << "KinoPaxSTARNoGoalBias execution time: " << executionTime << " seconds. Iterations: " << h_itr_
              << ". Tree Size: " << h_treeSize_ << ". Best Cost: " << h_minCost_ << std::endl;
}

float KinoPaxSTARNoGoalBias::planOptimize(float* h_initial, float* h_goal, float* d_obstacles_ptr, uint h_obstaclesCount)
{
    cudaEvent_t start, stop;
    float milliseconds = 0;
    cudaEventCreate(&start);
...
void KinoPaxSTARNoGoalBias::propagateFrontier(float* d_obstacles_ptr, uint h_obstaclesCount)
{
    ...
    if(h_frontierRepeatSize_ * h_activeBlockSize_ > (MAX_TREE_SIZE - h_treeSize_))
        {
            ...
            KinoPaxSTARNoGoalBias_propagateFrontier_kernel2<<<iDivUp(h_propIterations_ * h_frontierRepeatSize_, h_activeBlockSize_), h_activeBlockSize_>>>(
        ...
        {
            KinoPaxSTARNoGoalBias_propagateFrontier_kernel1<<<iDivUp(h_frontierRepeatSize_ * h_activeBlockSize_, h_activeBlockSize_), h_activeBlockSize_>>>(
              d_frontier_ptr_, d_activeFrontierRepeatIdxs_ptr_, d_treeSamples_ptr_, d_unexploredSamples_ptr_, h_frontierRepeatSize_,
              d_randomSeeds_ptr_, d_unexploredSamplesParentIdxs_ptr_, d_obstacles_ptr, h_obstaclesCount, graph_.d_activeSubVertices_ptr_,
              graph_.d_vertexScoreArray_ptr_, d_frontierNext_ptr_, graph_.d_counterArray_ptr_, graph_.d_validCounterArray_ptr_,
...
void KinoPaxSTARNoGoalBias::updateFrontier()
{
    ...
    // --- Update Frontier ---
    thrust::fill(d_activeFrontierRepeatCount_.begin(), d_activeFrontierRepeatCount_.end(), 0);
    KinoPaxSTARNoGoalBias_updateFrontier_kernel<<<iDivUp(h_frontierNextSize_ + h_treeSize_, h_blockSize_), h_blockSize_>>>(
...
/***************************/
void KinoPaxSTARNoGoalBias::getControlPathToGoal()
{
    ...
    KinoPaxSTARNoGoalBias_getControlPathToGoal_kernel<<<iDivUp(h_solSetSize_, h_blockSize_), h_blockSize_>>>(
...
void KinoPaxSTARNoGoalBias::writeExecutionTimeToCSV(double time)
{
