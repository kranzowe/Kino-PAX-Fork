...
// are fractions of B ITSELF. Both of the doors this paragraph used to describe as uncapped are gone:
// the region-best reactivation guarantee was folded permanently into the reactivation budget (see
// CS_DOORBIT_GUAR), and v3.4 folded the candidate-side OPTIMAL door into cost_frac's own budget the
// same way, after a toggle-driven sweep confirmed it helps -- see CS_DOORBIT_OPTIMAL and "THE FOUR
// DOORS" below.
//
// WHERE THAT LEAVES B. Nothing on the candidate side is uncapped any more, so B binds whenever the
// candidate pool for an iteration exceeds it and is a soft target (every door's cutoff solves to
// "admit everything offered") otherwise. budget_used against goal_frontier_size, read as a curve,
// is the direct measurement of which regime a given iteration is in.
// ================================================================================================
//
...
//
// THE THREE DOORS. (Two others used to run as their own unbudgeted arms and are both gone now,
// folded permanently into a budgeted door alongside them: GUARANTEE -- each active region's best
// node, unconditionally, if no optimal admission already covered it -- folded into CHEAPEST's
// reactivation histogram, so a region's best now has to win a react_frac * B slot like any other
// dormant node (see CS_DOORBIT_GUAR and Part B below); and OPTIMAL's own unconditional admission,
// folded into door 2's budget below -- v3.4 ran it as a toggle first, a sweep confirmed budgeting
// it helps, so the toggle is gone too.)
//
//   1. FRESHEST   explore_frac * B, taken from the least-populated regions.
//   2. CHEAPEST   cost_frac * B, taken from the smallest cost distances -- AND where an OPTIMAL
//                 candidate (distance 0, i.e. cost <= minCostsR1[r]) now competes: it always lands
//                 in bucket 0 (csCostBucket(0.0f, distMax) == 0), so it has first claim on this
//                 door's budget but is not admitted unconditionally the way it used to be. Still a
//                 stronger optimality guarantee than v1's region-best reactivation, which only put
//                 a region's best back AFTER the fact -- it just now shares CHEAPEST's budget
//                 rather than spending outside it. v3.3: no longer exclusive of door 1 -- see below.
//   3. DRAW       uniform over the rest of the tree at p = react_frac * B / treeSize.
//
// Plus a flat completeness floor on admission (v3.3, h_acceptFloor_ in the header), independent of
// both and reached only when neither fired for a candidate.
//
// FRESHEST AND CHEAPEST ARE A UNION, NOT A PRIORITY CHAIN. They select over the same candidate pool
// on independent signals, so a candidate can clear both. OPTIMAL, folded into CHEAPEST, can ALSO
// clear FRESHEST (v3.3: it used to return early on admission; now it falls through to the freshness
// check like any other candidate) -- but never clears CHEAPEST separately from OPTIMAL, since the
// two bits stay mutually exclusive by construction (see CS_DOORBIT_OPTIMAL). It is still one tree
// node, so every door that admits it is spent as ONE PROPAGATION BLOCK
// (nodeBlocks = popcount(doorMask) in Part A, no separate fan-out rule). Chaining FRESHEST/CHEAPEST
// as a priority order instead would make the later door's realised count depend on the earlier
// door's picks, so it would not meet its share.
//
...
CountingStars::CountingStars()
{
    ...
    // v3.3: the admission completeness floor. Same discipline as h_reactFloor_ above -- not a
    // tuning knob, its job is to be non-zero. See h_acceptFloor_ in the header for why its argument
...
void CountingStars::resetPlanner(float* h_initial, float* h_goal)
{
    ...
    // v3.3: 1, like every other node. The root is admitted by no door, so nothing else would ever
    // write its count -- but under door-count fan-out there is no boost left to seed it with; it is
    ...
    // h_reactFloor_ / h_acceptFloor_ above already rely on) -- and updateFrontier() divides by
    // h_fillIters_ every iteration, so a caller leaving it at 0 would produce NaN/Inf on the very
    // next run rather than the graceful MAX_ITER fallback this guards.
...
// anyway.
//
// regionCovered is still written by accept pass 2 -- the guarantee it used to be deduplicated
// against in Part B is gone; see d_regionCovered_ in the header.
...
// eligible for both and pass 2 resolves the overlap.
//
// v3.3: AN OPTIMAL CANDIDATE VOTES IN THE ORDINALITY HISTOGRAM TOO, because it now also competes
// for FRESHEST (pass 2 no longer returns early for it). v3.4: IT ALSO VOTES INTO COST BUCKET 0,
// permanently -- it competes for the SAME cost_frac * B cutoff CHEAPEST spends against, rather
// than being admitted for free (see CS_DOORBIT_OPTIMAL).
//
                                                 ...
                                                 float* candDistance, int* acceptHistogram)
{
    ...
    if(cost <= m)
        {
            // OPTIMAL. Permanently budgeted (v3.4) -- competes for CHEAPEST's cutoff rather than
            ...
            // scan touches it. This is the population MEASURED at distance 0, not what gets
            // admitted -- see h_optimalCount_ in the header.
            atomicAdd(&acceptHistogram[CS_HIST_OPT_SLOT], 1);
            // v3.3: OPTIMAL now also competes for FRESHEST, so it has to vote in the SAME ordinality
            // histogram non-optimal candidates vote in -- otherwise pass 2's cutoff would be solved
            // against a population this candidate was never measured in.
            atomicAdd(&acceptHistogram[CS_HIST_ORD_BASE + csOrdBucket(regionNodeCount[xR1])], 1);
            // v3.4: ALSO vote into cost bucket 0 -- csCostBucket(0.0f, distMax) == 0 for any distMax
            // (see the header), so no distance computation is needed here. This is an ADDITIONAL
            // vote, not a replacement of the two above: it makes this candidate compete for the SAME
            // cost_frac * B cutoff CHEAPEST already spends against.
            atomicAdd(&acceptHistogram[CS_HIST_COST_BASE + 0], 1);
            return;
        }
...
//
// ELIGIBLE == dormant (`!frontier`) and still expandable (`!goalSet`), full stop. A region's
// current best competes here like any other dormant node -- the exclusion this kernel used to
// apply for it (region bests came back through an unbudgeted GUARANTEE arm instead) is gone; see
// bestNodeIdxPerR1 in the header for what still reads that table (nothing, currently).
//
// Writing the flag makes the scan the SINGLE writer of the population Part B's cost arm selects
// from -- the same discipline that makes accept pass 1 the single measurer of the candidate pool.
//
...
/***************************/
// Admits against the cutoffs the host solved from pass 1's two histograms, ORing every door that
// fires into candDoor's bitmask:
//
//   OPTIMAL   distance == 0, routed through the SAME cutoff CHEAPEST uses (v3.4, permanent) --
//             NOT unconditional                                     bit = CS_DOORBIT_OPTIMAL
...
//   FLOOR     nothing above fired, and the flat acceptFloor roll succeeds   bit = CS_DOORBIT_FLOOR
//
// v3.3: OPTIMAL SETS ITS BIT AND FALLS THROUGH rather than returning -- it now also competes for
// FRESHEST exactly like any other candidate. v3.4: OPTIMAL no longer has automatic first claim on
// the budget -- a toggle-driven sweep confirmed that budgeting it (competing for CHEAPEST's cutoff
// rather than spending outside it) improves final cost / time-to-first-solution, so that is now the
// only behavior; there is no toggle any more. It STILL never sets CS_DOORBIT_CHEAP (see the
// !isOptimal guard below): the two bits stay mutually exclusive by construction, not because
// distance 0 is outside csCostBucket's domain any more -- it is now squarely inside it (bucket 0).
//
// FRESHEST AND CHEAPEST ARE A UNION, NOT A PRIORITY CHAIN. They select over the same candidate pool
// on independent signals -- region population and cost distance -- so a candidate can clear both,
// and OPTIMAL besides. It is still ONE tree node; every door that fires is one more bit in the mask,
// and Part A turns popcount(mask) into that many propagation blocks -- no separate fan-out rule.
//
// Ordering FRESHEST/CHEAPEST as a priority chain instead would make the second door's realised count
// depend on the first door's picks, so neither would meet its share.
//
// THE BOUNDARY ROLL IS WHAT MAKES EACH CUTOFF-BASED COUNT EXACT. The X-th candidate almost never
// falls on a bucket edge, and admitting the whole boundary bucket would overshoot by up to one
// bucket's width -- which, where thousands of candidates share a bucket, is most of a frontier. The
// roll spends the fractional remainder and nothing more. ONE curandState is loaded and stored even
// when a candidate sits on both boundaries; the two draws come off the same advanced state, which is
// exactly what two sequential draws mean.
//
// THE COMPLETENESS FLOOR is a SEPARATE, independent roll, reached only when mask == 0 after every
// cutoff check above -- see h_acceptFloor_ in the header for why this is a different argument from
// h_reactFloor_'s. It reloads randomSeeds rather than reusing the boundary draws' advanced state, so
// a candidate that needed every draw this kernel offers still costs at most two loads and two stores,
// not one four-draws-deep sequence tangled with the boundary logic above.
//
                                                 ...
                                                 unsigned long long* doorCounts)
{
    ...
    // --- FRESHEST: from the least-populated regions, exactly as before. ---
    int  ob        = csOrdBucket(regionNodeCount[xR1]);
    bool takeOrd   = (ob < ordCutoff);

    // --- CHEAPEST: cb == 0 == csCostBucket(0.0f, distMax) for an optimal candidate (see the
    // header) -- it competes for the SAME cutoff CHEAPEST uses, permanently (v3.4). ---
    int  cb        = isOptimal ? 0 : csCostBucket(candDistance[idx], distMax);
    bool takeCost  = (cb < costCutoff);

    // One state, at most two draws, one store -- even for a candidate sitting on both boundaries.
    // An optimal candidate at cb == 0 == costCutoff gets exactly the same fractional-remainder
    // roll any other bucket-0 candidate gets -- that IS competing for the budget.
    ...
    // --- OPTIMAL fires only for a candidate that just won the SAME cutoff/roll CHEAPEST resolved
    // above. NEVER also sets CS_DOORBIT_CHEAP (see the !isOptimal guard below): structural
    // exclusivity between OPTIMAL and CHEAP is preserved by construction. ---
    bool optimalAdmitted = isOptimal && takeCost;
    if(optimalAdmitted)
        {
            mask |= CS_DOORBIT_OPTIMAL;
            // Historical note: this used to also dedup Part B's region-best guarantee, which is
            // gone (see d_regionCovered_/CS_DOORBIT_GUAR in the header). Still marks the fact.
            regionCovered[xR1] = true;
            // CS_SLOT_COST now counts ADMITTED optimal candidates (h_admittedCost_), not MEASURED
            // ones (h_optimalCount_) -- strictly less whenever a budgeted candidate loses the
            // roll. (optimal_count - admitted_cost) is the starvation signal the v3.4 sweep
            // confirmed budgeting OPTIMAL is worth paying.
            atomicAdd(&doorCounts[CountingStars::CS_SLOT_COST], 1ULL);
        }

    if(takeOrd) { mask |= CS_DOORBIT_FRESH; atomicAdd(&doorCounts[CountingStars::CS_SLOT_EXPLORE], 1ULL); }
    // CS_DOORBIT_CHEAP / CS_SLOT_COSTDIST are for NON-optimal winners only: takeCost can now be
    // true for a budgeted-and-won optimal candidate without that meaning it went through CHEAPEST.
    if(!isOptimal && takeCost) { mask |= CS_DOORBIT_CHEAP; atomicAdd(&doorCounts[CountingStars::CS_SLOT_COSTDIST], 1ULL); }

    // THE COUNTERS OVERLAP DELIBERATELY. Two overlap terms: OPT_FRESH_BOTH for an ADMITTED optimal
    // candidate that also cleared FRESHEST, BOTH for a NON-optimal candidate that cleared both
    // FRESHEST and CHEAPEST. Both are gated on what actually ADMITTED (mask), not on the raw
    // isOptimal/takeCost results alone -- a budgeted-and-REJECTED optimal candidate that still wins
    // FRESHEST must not count as an OPTIMAL/FRESHEST overlap, and a budgeted-and-WON optimal
    // candidate that is also fresh must not double-count into BOTH (defined as the FRESHEST/
    // CHEAPEST overlap, not OPTIMAL/FRESHEST). The exact identity is spelled out where the doors
    // are declared in the header.
    if(optimalAdmitted && takeOrd)        atomicAdd(&doorCounts[CountingStars::CS_SLOT_OPT_FRESH_BOTH], 1ULL);
    if(!isOptimal && takeOrd && takeCost) atomicAdd(&doorCounts[CountingStars::CS_SLOT_BOTH], 1ULL);

    if(mask == CS_DOOR_NONE && acceptFloor > 0.0f)
        {
            // --- THE COMPLETENESS FLOOR. Only reached when nothing else fired. Independent roll,
            // its own load/store -- see the kernel doc above for why it does not reuse the boundary
            // draws' state. ---
            curandState seed = randomSeeds[idx];
            if(curand_uniform(&seed) < acceptFloor)
                {
                    mask |= CS_DOORBIT_FLOOR;
                    atomicAdd(&doorCounts[CountingStars::CS_SLOT_ACCEPT_FLOOR], 1ULL);
                }
            randomSeeds[idx] = seed;
        }

    candDoor[idx] = mask;
    if(mask == CS_DOOR_NONE)
        {
            // --- Rejected. Subtractive, like CleanCost's: propagate set the flag, admission leaves
            // it, and only rejection clears it. ---
            frontierNext[idx] = false;
        }
}
                               ...
                               unsigned long long* doorCounts)
{
    ...
    if(tid < frontierNextSize)
        {
            ...
            //
            // v3.3: DOOR-COUNT, FULL STOP. No region-thinness signal, no maxBlocks. A candidate
            // admitted by N doors gets N blocks -- door is the bitmask accept pass 2 built, so this
            // is the entire fan-out rule. Bounded small: at most 2 bits can be set here (OPTIMAL and
            // CHEAPEST never coexist, see CS_DOORBIT_OPTIMAL), so nodeBlocks is 1 or 2 for a Part A
            // insertion.
            nodeBlocks[x1TreeIdx] = __popc(door);

            // Update best-node index if this is the new region best. Currently unread (see
            // bestNodeIdxPerR1 in the header) -- Part B's guarantee, the table's one reader, is
            // gone. Kept as a cheap atomicExch on a branch that is already taken.
    ...
    // --- Part B: Re-activate existing tree nodes, filling what the budget has left ---
    else if(tid < frontierNextSize + treeSize)
        {
            ...
            // --- THE DRAW. Spent entirely on cost below (ARM 1), plus a completeness floor
            // (ARM 2). This keeps a sample of dormant nodes alive so the tree keeps deepening along
            // its cheap interior branches rather than only ever growing at its candidate edge --
            // region bests included: an unconditional, unbudgeted GUARANTEE arm used to reactivate
            // each region's best node ahead of this draw, for free; it is gone, folded permanently
            // into ARM 1's histogram (see CS_DOORBIT_GUAR), so a region's best now has to win a
            // react_frac * B slot like anything else.
            //
            // ONE curandState, at most two draws, one store -- the accept-pass-2 pattern.
            curandState seed = randomSeeds[treeIdx];

            // --- ARM 1: CHEAPEST. The whole react_frac * B budget, top-K by cost distance against
                    ...
                    if(take)
                        {
                            ...
                            nodeDoor[treeIdx]    = CS_DOORBIT_CHEAP;
...
void CountingStars::updateFrontier()
{
    ...
    if(h_frontierNextSize_ > 0)
        {
            ...
            //
            // THE SHARES ARE OF B ITSELF, NOT OF (B - admittedCost). v2 handed freshness a share of
            // what the optimal door had left, which made the freshness door's size depend on a count
            // it has nothing to do with. v3's three fractions are fixed, and OPTIMAL is folded into
            // the cost_frac * B share rather than spending on top of it (v3.4, permanent) -- so the
            // frontier is B*(explore + cost) + B*react + the floor's yield, with admittedCost
            // counted as part of cost_frac's share rather than as an overshoot on top of it.
            //
    ...
    // NO DESIGN-BUDGET SPLIT ANY MORE, AND NO REGION-KEYED FAN-OUT RULE EITHER (v3.3). Fan-out is
    // now decided per node in Part A as popcount(door) -- see the header. The only block constraint
    // left on the host is blockCeiling / blockScale in propagateFrontier, which is the buffer bound
    ...
    h_admittedFloor_         = (uint)h_doorCounts_[CS_SLOT_ACCEPT_FLOOR];
    ...
    // EXPLORE, COST and COSTDIST overlap by construction, so the identity that checks the door
    // counters against the compaction is
    //
    //     h_frontierNextSize_ == optimal + explore + costdist + floor - optFreshBoth - both
    //
    ...
    // plus the drawn reactivations that survived Part B's skips. Read against goal_frontier_size --
    // this is the claim the whole design rests on, in one column.
    // Part B has TWO arms (cheapest, then the completeness floor); reactivatedBest is kept in the
    // sum as a free runtime invariant -- it must read exactly 0 every iteration, since the arm that
    // used to fill it (the region-best guarantee) is gone. The benchmark's independent
    // thrust::count of frontier bits among the pre-existing tree must equal
    // reactivatedBest + reactivatedCost + reactivated -- a free check on both live arms, since one
    // side is a host scan and the other is device atomics.
