...
// ---- PAPER BENCHMARK V3 ----
//
// This file is a duplicate of paper_benchmark_v2.cu (same four planners, same three vehicle
// models, same MAX_ITERATIONS), with three changes: (1) MAX_TREE_SIZE drops to 1,000,000 (see
// run_paper_benchmark_v3.sh's write_config(), not 3,000,000); (2) MAX_TIME_MS -- the per-run
// wall-clock cap -- drops to 3 seconds, not 10; (3) each vehicle model now runs at ALL THREE
// discretizations (large/fine/tiny) instead of v2's one, each written to its own top-level output
// subfolder ("COARSE"/"FINE"/"TINY", passed in as a new CLI arg -- see main() below) rather than
// being distinguished only by filename. CS_RAMP_FILL_ITERS is rescaled accordingly -- see its own
// comment below. See run_paper_benchmark_v2.sh for the original MODEL 1->2->3 derivation history,
// unchanged here, including why Quad keeps its own native [0,100] workspace scale and how
