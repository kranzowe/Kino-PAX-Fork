...
{
    printf("\n========================================\n");
    printf("KINOPAXSTARCOSTPRUNE-NOSEED CAP SWEEP (pSeed=0): %s | Delta: %s | Regions: %d\n",
           environment_name.c_str(), deltaLabel.c_str(), NUM_R1_REGIONS);
    printf("========================================\n");

    // Same (cap, exp) grid as the seeded sweep above, so the two are directly comparable.
    // Sweep KinoPaxSTARcostprunenoseed over (max Syclop acceptance cap, cost-prune exponent).
    // cap=0 -> exploration acceptance always fails (pure cost-greedy); cap=1 -> uncapped.
    // costPruneExp k in the cost gate p=(minCostsR1/cost)^k (default 1.0; lower = gentler).
    const float       caps[]      = {0.0f, 0.4f, 1.0f, 0.0f, 0.0f};
    const float       exps[]      = {1.0f, 1.0f, 1.0f, 0.5f, 0.75f};
    const char* const capLabels[] = {"KinoPaxSTARcostprune_noseed_cap0",  "KinoPaxSTARcostprune_noseed_cap40",
...
int main(int argc, char* argv[])
{
    ...
    // tree-growth visualization (Data/Benchmarks/KinoPaxStarCost/viz/).
    ...
    const int NUM_KINOPAXSTARCOSTPRUNENOSEED_RUNS = 5;   // drives the KinoPaxSTARcostprunenoseed runner
    const int MAX_ITERATIONS       = 300;
    const float MAX_TIME_MS      = 10000.0f;  // 10 second timeout

    std::string outputDir = "Data/Benchmarks/KinoPaxStarCost";
    std::filesystem::create_directories(outputDir);

    printf("=======================================================\n");
    printf("    KINOPAXSTAR COST BENCHMARK\n");
    printf("=======================================================\n");
    ...
    printf("KinoPaxSTARcostprune_noseed: same sweep, pSeed=0 (no R2 seeding) x %d runs each\n", NUM_KINOPAXSTARCOSTPRUNENOSEED_RUNS);
    ...
    // --- KinoPaxSTARcostprune cap/exp sweep (R2 sub-region seeding on, as in the base variant) ---
    ...
    // --- KinoPaxSTARcostprunenoseed: identical sweep with the R2 seeding free pass removed (pSeed=0) ---
