...
// ---- PAPER BENCHMARK V3 (QUAD ONLY) ----
//
// This file is a duplicate of paper_benchmark_v3.cu (same MAX_TREE_SIZE=1,000,000, same
// MAX_TIME_MS=3s per-run cap, same 30 runs/planner, same COARSE/FINE/TINY output-subfolder
// mechanism), restricted to a SINGLE MODEL: Model 3 (12D Non-Linear Quad) only -- Double
// Integrator and Dubins Airplane are not built or run by this file at all. It also adds a FOURTH
// discretization, "medium" (MEDIUM/ output subfolder), between v3's existing large/fine points for
// Quad: `fine` (216,000 regions) didn't produce usable results and `large` (8,000) is too coarse
// to be a meaningful comparison, so this tries something in between -- W6/C2/V3 -> 46,656 regions,
// close to the geometric mean of large and fine (holding C_R1=2 fixed, same "consistent minimum
// attitude resolution" choice v3's own large/fine/tiny points for Quad already made). This is NOT
// part of the canonical large/fine/tiny set v3 and run_paper_benchmark.sh share -- it exists only
// in this file, for Quad, because neither of the canonical points worked well here.
//
// Everything else -- the four planners, CS_* CountingStars tuning constants, obstacle/start-goal
// W_SIZE scaling -- is identical to paper_benchmark_v3.cu. See run_paper_benchmark_v2.sh for the
// full MODEL 1->2->3 derivation history, including why Quad keeps its own native [0,100] workspace
// scale and how obstacles/start/goal get scaled into it (this file's main(), right after
// readObstaclesFromCSV).
//
                     ...
                     const std::string& deltaLabel)
{
    ...
    std::ostringstream filename;
    filename << outputDir << "/paper_benchmark_v3_" << timestamp.str() << "_summary.csv";
...
int main(int argc, char* argv[])
{
    ...
    // independently.
    std::string outputDir = "Data/Benchmarks/PaperBenchmarkV3/" + deltaFolder + "/" + envName;
    std::filesystem::create_directories(outputDir);

    printf("=======================================================\n");
    printf("    PAPER BENCHMARK V3 (QUAD ONLY)\n");
    printf("=======================================================\n");
