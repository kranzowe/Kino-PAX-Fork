...
//                            own on/off sweep confirmed the guard helps, and this is the
//                            re-tuned bufferSlope/bufferFloor/explore_frac/cost_frac point that
//                            followed. Replaces the earlier two-point (bf0.3/bf0.6), unguarded arm.
//
...
// reactivation, neither uncapped any more. The region-best reactivation guarantee that used to run
// unconditionally, outside any budget, is folded into the same cost-distance histogram/cutoff the
// CHEAPEST reactivation arm already uses -- a region's best dormant node has to win a
// react_frac * B slot like anything else now, rather than getting a free pass every iteration. The
// candidate-side OPTIMAL door (distance 0 from its region's minimum) went through the same fold,
// on the CANDIDATE side, one branch cut ago -- it ran as an on/off toggle there, a sweep confirmed
// budgeting it helps, and it is now equally permanent, with no toggle left for either side. See
// CS_DOORBIT_GUAR and CS_DOORBIT_OPTIMAL in CountingStars.cuh.
//
                               ...
                               int hopelessGuard)
{
    ...
    snprintf(buf, sizeof(buf), "CountingStars_bs%d_bf%d_ef%d_cf%d_hg%d",
             ...
             (int)lroundf(1000.0f * exploreFrac), (int)lroundf(1000.0f * costFrac), hopelessGuard);
    return std::string(buf);
}

std::string trueLabel(float syclopCap, int ancestorPrune)
{
    char buf[96];
...
RunResult benchmarkKinoPaxSTARTrue(
    ...
    float* h_initial, float* h_goal, float* d_obstacles, uint numObstacles, int maxIterations, float maxTimeMs,
    float syclopCap, int ancestorPrune, const std::string& label)
{
...
// ========================================================================
// CountingStars -- ONE fixed point: bufferSlope 1.2, bufferFloor 0.4, explore_frac 0.15,
// cost_frac 0.75, with THE HOPELESS GUARD PERMANENTLY ON (v3.5, h_hopelessGuard_) -- see
// runCountingStarsBenchmark() below for where it's set. Confirmed to help via
// countingstars_sweep.cu's own on/off sweep at an earlier operating point; slope/floor/ef/cf were
// then re-tuned with the guard on, since a ramp/budget tuned against the old, unguarded rule is
// not guaranteed to still be best once hopeless nodes stop competing for it.
// h_fillIters_ is set explicitly to CS_RAMP_FILL_ITERS, not left at its class default (MAX_ITER):
// the ramp's x = itr/h_fillIters_ must track the REAL run length, and at
...
RunResult benchmarkCountingStars(
    ...
    float* h_initial, float* h_goal, float* d_obstacles, uint numObstacles, int maxIterations, float maxTimeMs,
    float bufferSlope, float bufferFloor, float exploreFrac, float costFrac, int hopelessGuard,
...
void runCountingStarsBenchmark(
    ...
    int numRuns, int maxIterations, float maxTimeMs)
{
    // ONE fixed point, hopeless guard permanently ON: bufferSlope 1.2, bufferFloor 0.4,
    // explore_frac 0.15, cost_frac 0.75 -- re-tuned against countingstars_sweep.cu's own
    // bufferSlope/bufferFloor grid now that the guard is confirmed to help. Replaces the earlier
    // two-bufferFloor (0.3/0.6), unguarded arm.
    static const float BUFFER_SLOPE   = 1.2f;
    static const float BUFFER_FLOOR   = 0.4f;
    static const float EXPLORE_FRAC   = 0.15f;
    static const float COST_FRAC      = 0.75f;
    static const int   HOPELESS_GUARD = 1;

    printf("\n========================================\n");
    printf("COUNTINGSTARS: %s | Delta: %s | Regions: %d\n", environment_name.c_str(), deltaLabel.c_str(), NUM_R1_REGIONS);
    printf("========================================\n");

    const std::string label = countingStarsLabel(BUFFER_SLOPE, BUFFER_FLOOR, EXPLORE_FRAC, COST_FRAC, HOPELESS_GUARD);
    printf("  --- bufferSlope = %.2f, bufferFloor = %.2f, explore_frac = %.2f, cost_frac = %.2f, "
           "hopelessGuard = %d (%s) ---\n",
           BUFFER_SLOPE, BUFFER_FLOOR, EXPLORE_FRAC, COST_FRAC, HOPELESS_GUARD, label.c_str());
    CountingStars planner;
    for(int run = 0; run < numRuns; run++)
    {
        RunResult result = benchmarkCountingStars(planner, deltaLabel, environment_name, run,
                                             h_initial, h_goal, d_obstacles, numObstacles, maxIterations, maxTimeMs,
                                             BUFFER_SLOPE, BUFFER_FLOOR, EXPLORE_FRAC, COST_FRAC, HOPELESS_GUARD, label);
        printf("  Run %d/%d: %.3fs, %d itr, tree=%d, first_sol_itr=%d, cost=%.3f -> %.3f\n",
...
int main(int argc, char* argv[])
{
    ...
    printf("                CountingStars (bufferSlope 1.2, bufferFloor 0.4, explore_frac 0.15, cost_frac 0.75,\n");
    printf("                               hopelessGuard ON)\n");
    printf("=======================================================\n");
