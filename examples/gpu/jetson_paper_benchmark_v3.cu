...
// ---- JETSON PAPER BENCHMARK V3 ----
//
// This file is a duplicate of paper_benchmark_v3.cu (same four planners, same three vehicle
// models, same MAX_TREE_SIZE=1,000,000, same MAX_TIME_MS=3s per-run cap, same
// COARSE/FINE/TINY output-subfolder mechanism), adapted to actually run on a Jetson:
//
//   1. DISCRETIZATION FIXED TO "fine" FOR ALL THREE MODELS -- v3 ran all three deltas
//      (large/fine/tiny) per model; this file's run_jetson_paper_benchmark_v3.sh only defines the
//      three `fine` rows (one per model), so COARSE/ and TINY/ are simply never created. The
//      deltaFolder CLI arg and output-path construction below are UNCHANGED from v3 -- this file
//      does not know or care that only "fine" is active, it just does whatever the shell script
//      tells it to, same as v3.
//   2. RUN COUNT 30 -> 10 per planner (NUM_KPAX_RUNS etc. below) -- Jetson hardware is slower and
//      more memory-constrained, so a smaller run count keeps a full sweep tractable.
//   3. EXPLICIT CUDA ERROR CHECKING, ABSENT FROM v3. A `cudaErrorIllegalAddress` (documented
//      elsewhere in this repo as a real, previously-observed KPAX buffer-overshoot failure mode
//      specifically "under Quad on Jetson") does not necessarily crash the host process -- CUDA
//      errors are sticky and can leave a corrupted context silently failing every subsequent CUDA
//      call without ever segfaulting, which the external `timeout`/exit-code failure classification
//      in the shell script would never catch. Every one of the four per-iteration loops below now
//      calls `cudaGetLastError()` right after the existing `cudaEventSynchronize(iterStop)` (which
//      already forces a sync point for timing, so this adds no new synchronization cost) and exits
//      with a distinct code (2) on any error, printing the specific CUDA error string. See
//      run_jetson_paper_benchmark_v3.sh's RUN phase for how exit code 2 is classified in the
//      failure log as CUDA_ERROR, distinct from TIMEOUT/CRASHED_SIGx/ERROR.
//
// Everything else -- the four planners, model derivation history, obstacle/start-goal W_SIZE
// scaling, CS_* CountingStars tuning constants -- is identical to paper_benchmark_v3.cu. See that
// file and run_paper_benchmark_v2.sh for the full MODEL 1->2->3 history.
//
// NOTE: CS_RAMP_FILL_ITERS (below) is UNCHANGED from v3's value (210), because it was already
// re-derived there for MAX_TREE_SIZE=1,000,000 and the 3s cap, neither of which changes here --
// but the ORIGINAL 700-iterations empirical baseline it descends from was measured on cluster GPU
// throughput, not Jetson's. Worth confirming empirically after a first real Jetson run.
//
...
// How many iterations a run actually completes inside the wall-clock cap (empirical). The ramp's
// x = itr/fill_iters must track the REAL run length, not MAX_ITER: h_fillIters_ defaults to
// MAX_ITER (1000 in write_config()'s heredoc), but a run times out well short of that -- x would
// never reach 1 and B would never reach its ramp maximum for a run's entire duration. See
// benchmarkCountingStars() below, where this is assigned to planner.h_fillIters_ before
// resetPlanner().
//
// v3 VALUE, DERIVED FROM v2's: paper_benchmark_v2.cu calibrated 700 empirically at
// MAX_TREE_SIZE=3,000,000 and a 10s per-run cap. v3 runs at MAX_TREE_SIZE=1,000,000 and a 3s cap
// -- B's numerator (MAX_TREE_SIZE) already tracks the new config automatically via the formula
// above, but this denominator does not auto-scale, so it's rescaled by the wall-clock ratio:
// 700 * (3s / 10s) = 210. This is a LINEAR ESTIMATE, not independently re-benchmarked at v3's own
// tree size/cap -- worth confirming empirically after a first short v3 run (a smaller tree makes
// each iteration cheaper too, so the real achievable count at 3s may run somewhat higher than 210).
// This tool's own copy of the constant (paper_benchmark.cu, paper_benchmark_v2.cu, and
// countingstars_sweep.cu each keep their own copy too) -- a mismatched fill_iters would make the
// same (slope, floor) mean a different ramp in a different binary.
static const int CS_RAMP_FILL_ITERS = 210;
                     ...
                     const std::string& deltaLabel)
{
    ...
    std::ostringstream filename;
    filename << outputDir << "/jetson_paper_benchmark_v3_" << timestamp.str() << "_summary.csv";
    ...
    float maxTimeMs)
{
    ...
    while(itr < maxIterations)
    {
        ...
        // NEW IN JETSON VARIANT: cudaEventSynchronize above already forces a sync point, so this
        // adds no extra cost. A sticky CUDA error (e.g. cudaErrorIllegalAddress) does not always
        // crash the host process -- catch it explicitly rather than let it silently corrupt every
        // subsequent CUDA call for the rest of this run.
        {
            cudaError_t launchErr = cudaGetLastError();
            if(launchErr != cudaSuccess)
            {
                fprintf(stderr, "\n*** CUDA ERROR: KinoPaxPlus | env=%s run=%d itr=%d | %s ***\n",
                        environment.c_str(), runNumber, itr, cudaGetErrorString(launchErr));
                fflush(stderr);
                exit(2);
            }
        }
    ...
    float maxTimeMs)
{
    ...
    while(itr < maxIterations)
    {
        ...
        // NEW IN JETSON VARIANT: see benchmarkKinoPaxPlus's copy of this check for why. KPAX is
        // the planner with a DOCUMENTED history of cudaErrorIllegalAddress specifically under Quad
        // on Jetson (the buffer-overshoot bug referenced in run_paper_benchmark_v2.sh's header), so
        // this check matters most here.
        {
            cudaError_t launchErr = cudaGetLastError();
            if(launchErr != cudaSuccess)
            {
                fprintf(stderr, "\n*** CUDA ERROR: KPAX | env=%s run=%d itr=%d | %s ***\n",
                        environment.c_str(), runNumber, itr, cudaGetErrorString(launchErr));
                fflush(stderr);
                exit(2);
            }
    ...
    const std::string& label)
{
    ...
    while(itr < maxIterations)
    {
        ...
        // NEW IN JETSON VARIANT: see benchmarkKinoPaxPlus's copy of this check for why.
        {
            cudaError_t launchErr = cudaGetLastError();
            if(launchErr != cudaSuccess)
            {
                fprintf(stderr, "\n*** CUDA ERROR: KinoPaxSTARTrue | env=%s run=%d itr=%d | %s ***\n",
                        environment.c_str(), runNumber, itr, cudaGetErrorString(launchErr));
                fflush(stderr);
                exit(2);
            }
    ...
    const std::string& label)
{
    ...
    // But MAX_ITER itself is now a poor stand-in for "the run is over": at this build's
    // MAX_TREE_SIZE and MAX_TIME_MS wall-clock cap, a real run completes far fewer iterations than
    // MAX_ITER (1000) -- x = itr/h_fillIters_ would never reach 1 and B would never reach its ramp
    // maximum for a run's entire duration. CS_RAMP_FILL_ITERS is that real run length instead --
    // see its own definition above for v3's specific derivation (210, at MAX_TREE_SIZE=1,000,000
    // and a 3s cap).
    ...
    while(itr < maxIterations)
    {
        ...
        // NEW IN JETSON VARIANT: see benchmarkKinoPaxPlus's copy of this check for why.
        {
            ...
            if(launchErr != cudaSuccess)
            {
                fprintf(stderr, "\n*** CUDA ERROR: CountingStars | env=%s run=%d itr=%d | %s ***\n",
                        environment.c_str(), runNumber, itr, cudaGetErrorString(launchErr));
                fflush(stderr);
                exit(2);
            }
...
int main(int argc, char* argv[])
{
    ...
    // NEW IN v3: the discretization output folder ("COARSE"/"FINE"/"TINY"), since v3 runs all
    // three deltas per model instead of v2's one -- see outputDir below. Positional, not a flag,
    // so it must come before the flag-scan loop; that loop's start index shifts from 4 to 5
    // accordingly.
    std::string deltaFolder   = (argc > 4) ? argv[4] : "unknown";
    ...
    //
    // --only-<planner> runs exactly that one planner and nothing else. run_paper_benchmark_v3.sh
    // invokes this binary once PER PLANNER (see its header for why: a true CUDA hang has no
    ...
    for(int i = 5; i < argc; i++)   // starts at 5, not 4 -- argv[4] is deltaFolder, not a flag
    {
    ...
    const int NUM_KPAX_RUNS        = 10;   // was 30 in paper_benchmark_v3.cu -- Jetson-reduced
    const int NUM_KINOPAXPLUS_RUNS = 10;   // drives the KinoPaxPlus runner
    const int NUM_TRUE_RUNS        = 10;    // drives the KinoPaxSTARTrue anc0/anc1 pair
    const int NUM_CS_RUNS          = 10;    // drives the CountingStars fixed point
    const int MAX_ITERATIONS       = 1000;
    const float MAX_TIME_MS      = 3000.0f;  // 3 second per-run timeout (was 10s in v2)

    // Per-(discretization, environment) subfolder -- NEW IN v3: deltaFolder ("COARSE"/"FINE"/
    // "TINY") splits output at the top level since all three deltas now run per model, with
    // environment nested inside as before so each (delta, environment) pair can still be plotted
    // independently.
    // Separate root from paper_benchmark_v3.cu's "PaperBenchmarkV3" -- Jetson runs use their own
    // NUM_*_RUNS (10, not 30) and hardware, so pooling into the same directory tree risks a
    // Jetson run's run0-9 CSVs silently overwriting a cluster run's run0-9 CSVs (both start
    // numbering at 0). Same COARSE/FINE/TINY/<env> nesting convention as v3, just a different root.
    std::string outputDir = "Data/Benchmarks/PaperBenchmarkV3Jetson/" + deltaFolder + "/" + envName;
    std::filesystem::create_directories(outputDir);

    printf("=======================================================\n");
    printf("    JETSON PAPER BENCHMARK V3\n");
    printf("=======================================================\n");
    printf("Delta label:    %s\n", deltaLabel.c_str());
    printf("Delta folder:   %s\n", deltaFolder.c_str());
    ...
    // logic yet, tiny's own region count was reduced instead (MODEL_DELTA_C_R1S/MODEL_DELTA_V_R1S
    // in run_paper_benchmark_v3.sh, 592,704 -> 373,248 for Quad specifically, inherited unchanged
    // from run_paper_benchmark_v2.sh) to test whether a smaller discretization
