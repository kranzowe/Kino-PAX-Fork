...
{
    RunResult result;
    result.delta_label = "KinoPaxSTARNoPrune";
    result.build_delta = deltaLabel;
    result.environment = environment;
    result.run_number = runNumber;
    result.first_solution_iteration = -1;
...
{
    printf("\n========================================\n");
    printf("KINOPAXSTARNOPRUNE: %s | Delta: %s | Regions: %d\n",
           environment_name.c_str(), deltaLabel.c_str(), NUM_R1_REGIONS);
    printf("========================================\n");
...
// ========================================================================
// KinoPaxSTARNoPruneNoSpatialHash benchmark + runner (KinoPaxSTARNoGoalBias with the
// spatial-hash collision grid removed; identical search, direct propagateAndCheck
// collision path. Same instrumentation as KinoPaxSTARNoGoalBias).
// ========================================================================
RunResult benchmarkKinoPaxSTARNoPruneNoSpatialHash(
    KinoPaxSTARNoPruneNoSpatialHash& planner,
    const std::string& deltaLabel,
    const std::string& environment,
    int runNumber,
    float* h_initial,
...
{
    RunResult result;
    result.delta_label = "KinoPaxSTARNoPruneNoSpatialHash";
    result.build_delta = deltaLabel;
    result.environment = environment;
    result.run_number = runNumber;
    result.first_solution_iteration = -1;
...
void runKinoPaxSTARNoPruneNoSpatialHashBenchmark(
    const std::string& environment_name,
    float* h_initial,
    float* h_goal,
    float* d_obstacles,
...
{
    printf("\n========================================\n");
    printf("KINOPAXSTARNOPRUNENOSPATIALHASH: %s | Delta: %s | Regions: %d\n",
           environment_name.c_str(), deltaLabel.c_str(), NUM_R1_REGIONS);
    printf("========================================\n");

    {
        ...
        for(int run = 0; run < numRuns; run++)
        {
            RunResult result = benchmarkKinoPaxSTARNoPruneNoSpatialHash(planner, deltaLabel, environment_name, run,
                                                 h_initial, h_goal, d_obstacles,
                                                 numObstacles, maxIterations, maxTimeMs);
            printf("  Run %d/%d: %.3fs, %d itr, tree=%d, first_sol_itr=%d, cost=%.3f -> %.3f\n",
                   run + 1, numRuns, result.total_time_seconds, result.total_iterations,
...
int main(int argc, char* argv[])
{
    ...
    std::string outputDir = "Data/Benchmarks/KinoPaxStarLarge";
