...
class KinoPaxSTARWeightedCost : public Planner
{
public:
    /**************************** CONSTRUCTORS ****************************/
    ...
    // --- Ancestor-pruning tunables (KinoPaxPlus port) ---
    //   h_ancestorPrune_     0 = off, 1 = node-only, 2 = memoized ancestor chain
    //   h_dormancyThreshold_ iterations a pruned-but-region-best node survives before un-pruning
    //   h_ancestorTol_       slack: cost > minCostsR1[r] * (1 + tol). 0 == KinoPaxPlus's test.
...
// found. Uses one parent lookup rather than an O(depth) chain walk -- see the .cu for why that is
// exactly equivalent. One thread per tree node.
