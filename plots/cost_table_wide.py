...
def regions_for_model(model_id: int) -> list:
    ...
    discretization token. No Jetson rows on this branch -- see the module docstring. Zephyr's
...
def render_latex(subtitle: str, sections: dict, regions: list, model_id: int) -> str:
    ...
    lines = [
        r"\begin{table*}[htbp]",
        r"\centering",
        rf"\caption{{Cost Comparison --- {subtitle} (Short Timeout: 1M-node / 3s)}}",
        rf"\label{{tab:cost_comparison_{sanitize_name(subtitle).lower()}_short}}",
...
def main() -> None:
    ...
    for model_id in MODEL_IDS:
        ...
        base_name = f"cost_table_wide_m{model_id}_{sanitize_name(MODEL_NAMES_LOCAL[model_id])}_short"
