...
module docstring for the full explanation (same dataset): discretizationFINE/FINE is both 6D
systems, Models 1 and 2 (paper_benchmark_v3 pipeline, tags filenames by model) -- Model 3's own
fine-resolution Jetson attempt is excluded, since Kino-PAX+ failed almost entirely there (0/20 in
two of three environments per that run's failures.log). Model 3 gets a "Jetson (42K)" row instead
(JETSON_DIR/NEWQUAD_MEDIUM, same tagged pipeline, "medium" discretization token) -- new hardware
data at a genuinely new discretization, replacing this table's earlier, much weaker "Jetson
(Coarse)" attempt (the older, untagged pre-v2 pipeline). A region whose folder doesn't exist yet
prints as "--" rather than erroring.
...
def regions_for_model(model_id: int) -> list:
    ...
    Zephyr's "Coarse" folder's on-disk token is "large" (the harness's own coarsest-resolution
    label), not "coarse". Every region here uses the tagged pipeline (load_runs) now that Model
    3's Jetson row is NEWQUAD_MEDIUM instead of the old untagged discretizationCOARSE attempt."""
