"""Final-cost ratio scatter: Kino-PAX+ (y-axis) vs every other planner (x-axis).

Exactly the same scatter as ttfs_ratio_scatter.py, but the axes are final path cost instead of
time to first solution. One figure per (cost metric x model) -- length and effort are separate,
non-comparable sweeps (different units) and must never be pooled together, unlike time-to-first-
solution which is metric-independent. Color = algorithm, shape = discretization (only "tiny"
exists today; "fine"/"coarse" are picked up automatically once those sweeps are added alongside
it). Every algorithm compared within one (environment, discretization) cell shares Kino-PAX+'s
mean cost as its y-value, so they land on a shared horizontal line -- that line is labeled
directly with the environment name. The "empty" environment is excluded (not interesting -- every
planner solves it trivially). The y=x line marks "as good as Kino-PAX+". See
...
Edit DATASET_DIR / OUT_DIR below to point at the dataset you want to plot, then run:
    python plots/cost_ratio_scatter.py
"""
...
def plot_model_cost(table: pd.DataFrame, model_id: int, metric: str, discretization_labels: list[str],
                     out_path: str) -> None:
    ...
    ax.set_xlabel(f"Final Cost — other algorithms ({metric_title})")
    ax.set_ylabel(f"Final Cost — Kino-PAX+ ({metric_title})")
    ax.set_title(
        f"Final Cost vs Kino-PAX+ — {MODEL_NAMES[model_id]} (m{model_id}), {metric_title}\n"
...
def main() -> None:
    ...
    for metric in COST_METRICS:
        for model_id in MODEL_IDS:
            ...
            if n_total_runs == 0:
                print(f"  [{metric} m{model_id} {model_name}] no run files found -- skipping.")
            else:
                any_data = True

            base_name = f"cost_ratio_{metric}_m{model_id}_{sanitize_name(model_name)}"
            csv_path = os.path.join(OUT_DIR, f"{base_name}.csv")
            table.to_csv(csv_path, index=False)
            png_path = os.path.join(OUT_DIR, f"{base_name}.png")
            plot_model_cost(table, model_id, metric, discretization_labels, png_path)
            print(f"  [{metric} m{model_id} {model_name}] wrote {csv_path}, {png_path} + .svg "
                  f"({n_total_runs} runs)")
