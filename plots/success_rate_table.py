...
def render_latex(subtitle: str, sections: dict, regions: list) -> str:
    ...
    lines = [
        r"\begin{table}[htbp]",
        r"\centering",
        rf"\caption{{Success Rate --- {subtitle} (Short Timeout: 1M-node / 3s)}}",
        rf"\label{{tab:success_rate_{sanitize_name(subtitle).lower()}_short}}",
...
def main() -> None:
    ...
    for model_id in MODEL_IDS:
        ...
        base_name = f"success_rate_table_m{model_id}_{sanitize_name(MODEL_NAMES_LOCAL[model_id])}_short"
